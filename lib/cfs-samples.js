...
export const MAIN_URL = `${CFS_BASE}/tc_chi/unsat_samples/unsat_samples.html`;
...
export function sampleKey(sample) {
  return `${sample.date}|${sample.title}|${sample.url ?? ""}`;
}
...
/** Fetch samples shown on the main CFS page (current month). */
export async function fetchMainPageSamples() {
  const html = await fetchHtml(MAIN_URL);
  return parseSamples(html);
}

/** Fetch recent samples for the homepage list. */
